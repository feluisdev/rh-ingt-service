...
public class CreateEnquadramentoCommandHandler
    ...
    public ResponseEntity<Map<String, ?>> handle(CreateEnquadramentoCommand command) {
        ...
        // --- Validação cruzada: contrato activo obrigatório ---
        ...
        // --- Validação cruzada: data de início dentro do período do contrato ---
        ...
        // --- Determinar se o tipo de contrato exige estrutura de carreira ---
        boolean requiresCareer = false;
        if (contrato.getContractTypeId() != null) {
            var contractType = contractTypeRepository.findById(
                    ContractTypeId.from(contrato.getContractTypeId()));
            requiresCareer = contractType.map(ct -> ct.isRequiresCareerStructure()).orElse(false);
        }

        // --- Validar carreira/categoria/escalão consoante o tipo de vínculo ---
        UUID careerId = null;
        ...
        if (requiresCareer) {
            ...
            // Campos opcionais: validar apenas se fornecidos
            if (dto.getCareerId() != null && !dto.getCareerId().isBlank()) {
                var career = careerRepository.findById(CareerId.from(dto.getCareerId()))
                        .orElseThrow(() -> IgrpResponseStatusException.of(HttpStatus.UNPROCESSABLE_ENTITY, "Carreira não encontrada: " + dto.getCareerId()));
                if (!Boolean.TRUE.equals(career.getIsActive()))
        ...
        // --- Cargo e unidade orgânica (sempre obrigatórios) ---
        ...
        // --- Encerrar enquadramento anterior se existir ---
        ...
        if (currentEnquadramento.isPresent()) {
            if (!dto.getDataInicio().isAfter(currentEnquadramento.get().getDataInicio()))
                throw IgrpResponseStatusException.of(HttpStatus.UNPROCESSABLE_ENTITY,
                        "A data de início deve ser posterior à data de início do enquadramento actual (" + currentEnquadramento.get().getDataInicio() + ").");
            var prev = currentEnquadramento.get();
            prev.encerrar(dto.getDataInicio().minusDays(1));
            enquadramentoRepository.save(prev);
        }

        // --- Função (opcional): validar compatibilidade com cargo ---
