...
@Component("colabsRegistarDocumentoLicencaCommandHandler")
@RequiredArgsConstructor
public class RegistarDocumentoLicencaCommandHandler
        implements CommandHandler<RegistarDocumentoLicencaCommand, ResponseEntity<DocumentoUploadResponseDTO>> {
    ...
    @IgrpCommandHandler
    public ResponseEntity<DocumentoUploadResponseDTO> handle(RegistarDocumentoLicencaCommand command) {
        var funcionarioId = FuncionarioId.from(command.getFuncionarioId());
        funcionarioRepository.findById(funcionarioId)
                .orElseThrow(() -> IgrpResponseStatusException.notFound(
                        "Funcionário não encontrado: " + command.getFuncionarioId()));

        var licencaId = LicencaMobilidadeId.from(command.getLicencaId());
        var licenca = licencaRepository.findById(licencaId)
                .orElseThrow(() -> IgrpResponseStatusException.notFound(
                        "Licença não encontrada: " + command.getLicencaId()));

        if (!licenca.getFuncionarioId().equals(funcionarioId))
            throw IgrpResponseStatusException.notFound("Licença não encontrada: " + command.getLicencaId());
