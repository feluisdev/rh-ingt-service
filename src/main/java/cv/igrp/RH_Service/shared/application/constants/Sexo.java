...
public enum Sexo implements IgrpEnum<String> {
  ...
  Sexo(String code, String description) {
    this.code = code;
  ...
  */
  private static final Map<String, Sexo> CODE_MAP = Arrays.stream(values())
          .collect(Collectors.toMap(Sexo::getCode, Function.identity()));
  ...
  */
  public static Optional<Sexo> fromCode(String code) {
  ...
  */
  public static Sexo fromCodeOrThrow(String code) {
    return fromCode(code).orElseThrow(() -> new IllegalArgumentException("Invalid Sexo for this code: " + code));
  }
  ...
  */
  public static Map<String, String> codeDescriptionMap() {
    return CODE_MAP.values().stream().collect(Collectors.toMap(Sexo::getCode, Sexo::getDescription));
  }
