...
public class CreateSiadapEvaluationCommand implements Command {
    ...
    // controlador (ComplianceController.createEvaluation: new
    // CreateSiadapEvaluationCommand(request)), fora do alcance de qualquer validação de bean do
    // Spring, logo as restrições que aqui estiveram nunca cascatearam. A validação real do corpo já
    // acontece no parâmetro @Valid @RequestBody CreateSiadapEvaluationRequestDTO do próprio
