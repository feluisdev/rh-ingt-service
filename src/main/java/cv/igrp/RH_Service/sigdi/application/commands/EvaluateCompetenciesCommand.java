...
@NoArgsConstructor
@AllArgsConstructor
public class EvaluateCompetenciesCommand implements Command {
    ...
    // controlador (ComplianceController.evaluateCompetencies: new
    // EvaluateCompetenciesCommand(request)), fora do alcance de qualquer validação de bean do
    // Spring, logo as restrições que aqui estiveram nunca cascatearam. A validação real do corpo já
    // acontece no parâmetro @Valid @RequestBody List<CompetencyItemDTO> competencies do próprio
    ...
    private EvaluateCompetenciesRequestDTO body;
}
