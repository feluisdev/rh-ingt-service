...
public enum NotificationChannel implements IgrpEnum<String> {
  ...
  */
  private static final Map<String, NotificationChannel> CODE_MAP = Arrays.stream(values())
          .collect(Collectors.toMap(NotificationChannel::getCode, Function.identity()));
  ...
  */
  public static NotificationChannel fromCodeOrThrow(String code) {
    return fromCode(code).orElseThrow(() -> IgrpResponseStatusException.of(HttpStatus.BAD_REQUEST, "Invalid NotificationChannel for this code: " + code));
  }
  ...
  */
  public static Map<String, String> codeDescriptionMap() {
    return CODE_MAP.values().stream().collect(Collectors.toMap(NotificationChannel::getCode, NotificationChannel::getDescription));
  }
