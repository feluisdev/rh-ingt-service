 ...
 * looks at evaluations in {@link EvaluationPhase#IN_PROGRESS} and acts when the year's individual
 * {@code SIADAP_SELF_EVAL} window comes back non-empty. This scheduler looks at evaluations in
 * {@link EvaluationPhase#SELF_EVALUATION} and acts when the same query, for the same
 * {@link Purpose#SIADAP_SELF_EVAL}, {@code isEmpty()}. The two conditions are mutually exclusive
 * for a given year, so running both jobs at the same default hour introduces no coordination
 * problem: for any one evaluation, at most one of the two schedulers can find its guard
 * satisfied.
 * <p>
 * <b>Absence and expiry are the same fact, by design.</b> This class never asks a
 * {@link SiadapEvaluation} about its own dates. It asks
 * {@link PaaSubmissionPeriodRepository#findActiveByTypeAndYearAndPurpose} for the year in
 * question, backed by {@code PaaSubmissionPeriod.isActiveToday()}. A year whose evaluations are
 * in {@code SELF_EVALUATION} and whose {@code SIADAP_SELF_EVAL} window was closed by RH, expired
 * naturally, or was <b>never configured at all</b>, produces the exact same
 * {@link Optional#empty()} from that query -- and is therefore eligible for tacit acceptance on
 * the very first run after the evaluations entered that phase. This is the unification success
 * criterion 5 requires, taken to its consequence: there is no "grace period" for a window that
 * was simply never created, because absence of a window and an expired window are indistinguishable
 * for this predicate.
 * <p>
 ...
 * The "what day is it" decision is made once, downstream, inside
 * {@code PaaSubmissionPeriodRepositoryImpl}, which reads today's date with
